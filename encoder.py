...
def add_embedding(model, inputs, num_steps):
    """
        @model:
        @inputs:        (int32)         batch_size x num_steps
        @num_steps:     (int)           number of steps for encoder
    ...
    with tf.variable_scope('Embedding'):
        ...
        output = tf.split(output, tf.ones(num_steps, dtype=tf.int32), axis=1)
...
def add_encoding_layer(model, inputs, initial_state, num_steps):
    """
        @model:
...
def add_encoding(model, inputs, initial_states, num_steps):
    """
        @model:
    ...
    for layer in xrange(config.en_layers):
        output, state = add_encoding_layer(model, output, initial_states[layer], num_steps)
